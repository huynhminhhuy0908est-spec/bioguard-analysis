    ...
    def transform(self, frame):
        ...
        if results.pose_landmarks:
            ...
            try:
                ...
                cv2.putText(img, f"Goc: {angle} do", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                
                if angle < 90:
                    cv2.putText(img, "NGUY HIEM: Gap goi qua gat!", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
                    cv2.circle(img, tuple(np.multiply(knee, [img.shape[1], img.shape[0]]).astype(int)), 15, (0, 0, 255), -1)
                elif angle < 120:
                    cv2.putText(img, "CANH BAO: Chiu tai lon", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
                else:
                    cv2.putText(img, "AN TOAN", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
...
input_source = st.sidebar.radio("Nguồn Video:", ("Camera Trực Tiếp (Điện thoại/PC)", "Tải Video lên"))
target_leg = st.sidebar.radio("Chân cần phân tích:", ("Chân Trái", "Chân Phải"))

if input_source == "Camera Trực Tiếp (Điện thoại/PC)":
    ...
    if webrtc_ctx.video_transformer:
        webrtc_ctx.video_transformer.target_leg = target_leg
else:
